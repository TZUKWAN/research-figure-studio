...
import {} from '@genoffice/agent-core'
import type { ModelCapabilityProfile } from '@genoffice/ai-provider'
import {
...
import { AGENT_SYSTEM_PROMPT, RESEARCH_AGENT_SYSTEM_PROMPT } from '../../shared/prompt-defaults'
import {} from '../../shared/prompt-protocol'
...
` +
    ...
    case 'create_research_figure': {
      ...
      // one atomic txn → post-write verification). The legacy inline
      // per-element path was deleted — it must not come back.
      return await executeCreateResearchFigure({
        access,
        call,
        ...(state
          ? {
              setDurableFigureState: (durable: DurableFigureState) => {
                state.durableFigureState = durable
              },
            }
          : {}),
        ...(signal ? { signal } : {}),

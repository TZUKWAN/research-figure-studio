...
/* Keep in sync with the main-process open-dialog filter (OPEN_DIALOG_EXTENSIONS). */
const OPEN_LOCAL_EXTENSIONS = '.pptx'
...
// Opens the settings modal (AI model API keys, language, theme, save location).
...
export function Home() {
  ...
  const importIntoProject = async (projectId: string) => {
    const imported = await window.aiOfficeProject?.importFiles(projectId)
    if (imported && imported.length > 0) refresh()
  }
  ...
  // ── New Slides file (passes projectId when a project is selected) ──
  ...
  function renderQuickCards() {
    return (
      <div className="quick-cards">
        <button className="quick-card" onClick={() => void handleNewSlide()}>
          <FileBadge ext="pptx" size={26} />
          <span className="quick-text">
            <span className="quick-title">{t('newSlide')}</span>
          </span>
        </button>
        <button
          ...
          <div className="recents-toolbar">
            <div className="quick-cards">
              ...
              <button
                className="quick-card"
                onClick={() => {
                  if (selectedProjectId) void importIntoProject(selectedProjectId)
                }}
                data-tip={OPEN_LOCAL_EXTENSIONS}
              >
                <span className="quick-folder">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                    <path
                      d="M8 2.5v7m0 0L5.2 6.7M8 9.5l2.8-2.8M3 11v1.5A1.5 1.5 0 0 0 4.5 14h7a1.5 1.5 0 0 0 1.5-1.5V11"
                      stroke="currentColor"
                      strokeWidth="1.3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                ...
                <span className="quick-text">
                  <span className="quick-title">{t('importIntoProject')}</span>
                </span>
              </button>
            </div>
  ...
  function renderGlobalContent() {
    return (
      <main className="content">
        ...
        >
          <div className="recents-toolbar">
            {selectedPaths.length === 0 && <div className="quick-cards">{renderQuickCards()}</div>}
    ...
    <div className="home">
      <aside className="sidebar">
        <div className="sidebar-logo">
          <span className="logo-wordmark">Metis Diagram</span>
        </div>

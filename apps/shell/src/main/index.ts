...
const OPEN_DIALOG_EXTENSIONS = ['pptx']
...
function registerHomeIpc(): void {
  ipcMain.handle(HOME_CHANNELS.recents, (_event, query: unknown): RecentPage =>
    pageRecentPaths(readSlidesRecentFiles(), slidesQuery(query), new Set(readStarredSlides())),
  )
  ...
  ipcMain.handle(HOME_CHANNELS.toggleStar, (_event, path: unknown) => {
    if (typeof path !== 'string' || !PPTX_RE.test(path) || !existsSync(path)) return
    const starred = readPathList(starredPath())
    const next = starred.includes(path)
      ? starred.filter((item) => item !== path)
      : [...starred, path]
  ...
  ipcMain.handle(HOME_CHANNELS.removeRecent, (_event, paths: unknown) => {
    removeSlidesRecentFiles(stringPaths(paths))
  })
  ...
  ipcMain.handle(
    ...
    (_event, path: unknown, newName: unknown): RenameResult => {
      ...
      if (!PPTX_RE.test(path) || !PPTX_RE.test(name) || /[\\/:]/.test(name))
        return { ok: false, error: tm('errBadName') }
  ...
  ipcMain.handle(HOME_CHANNELS.getDefaultSaveDir, (): string => configuredDefaultSaveDir(app))
  ...
  ipcMain.handle(PROJECT_CHANNELS.list, () => getShellProjectStore().listProjectsSummary())
  ipcMain.handle(PROJECT_CHANNELS.files, (_event, args: { projectId: string }) =>
    getShellProjectStore().listProjectFiles(args.projectId),
  )
  ipcMain.handle(PROJECT_CHANNELS.create, (_event, args: { name: string }) => {
    const store = getShellProjectStore()
    const project = store.createProject(args.name)
    return store.listProjectsSummary().find((summary) => summary.id === project.id) ?? project
  })
  ipcMain.handle(PROJECT_CHANNELS.rename, (_event, args: { id: string; name: string }) => {
    getShellProjectStore().renameProject(args.id, args.name)
  })
  ipcMain.handle(PROJECT_CHANNELS.delete, (_event, args: { id: string }) => {
    getShellProjectStore().deleteProject(args.id)
  })
  ipcMain.handle(
    PROJECT_CHANNELS.moveFile,
    (_event, args: { filePath: string; projectId: string }) => {
      getShellProjectStore().moveFileToProject(args.filePath, args.projectId)
    },
  )
  ipcMain.handle(PROJECT_CHANNELS.importFiles, async (_event, args: { projectId: string }) => {
    const store = getShellProjectStore()
    const win = shellWindow && !shellWindow.isDestroyed() ? shellWindow : undefined
    if (!win) return []
    const result = await showOpenDialogWithMemory(dialog, win, {
      properties: ['openFile', 'multiSelections'],
      filters: [{ name: tm('filterSupported'), extensions: OPEN_DIALOG_EXTENSIONS }],
    ...
    for (const filePath of result.filePaths) {
      try {
        store.moveFileToProject(filePath, args.projectId)
  ...
  ipcMain.handle(PROJECT_CHANNELS.timeline, (_event, args: { projectId: string; limit?: number }) =>
    getShellProjectStore().getProjectTimeline(args.projectId, args.limit ?? 20),
  )
}

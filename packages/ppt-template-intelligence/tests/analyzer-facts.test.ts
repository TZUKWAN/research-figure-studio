/**
 * GOAL §十二-十五 + §十九: analyzer facts expansion verified against a real
...
describe('analyzer observed-facts expansion (GOAL §十二-十五)', () => {
  it.skipIf(!HAS_GORDEN)('extracts fonts, layout parts and extended shape facts', async () => {
    ...
    // §十三: theme font scheme read from theme1.xml
    expect(facts.themeFonts.majorLatin || facts.themeFonts.minorLatin).toBeTruthy()
    // §十二: layout parts observed for slides
    expect(facts.pages.some((p) => p.layoutPart && p.layoutPart.includes('slideLayout'))).toBe(
      true,
    )
    // §十二: z-order + rotation + anchor facts present on at least one shape
  ...
  it.skipIf(!HAS_GORDEN)('builds geometry-accurate capacity and figure slots', async () => {
    ...
    // §十四: capacity carries box geometry + derived line stats
    ...
    // §十三: observed font profile
    expect(def.style.fontProfile).toBeDefined()
    expect(def.style.fontProfile!.theme.majorLatin ?? def.style.fontProfile!.theme.minorLatin).toBeTruthy()
    // §十九: figure slots exist for EVERY observed picture/chart shape
  ...
  it.skipIf(!HAS_GORDEN)('real-layout measure upgrades confidence when the family is installed', async () => {
    const bytes = readFileSync(join(GORDEN_DIR!, 'minimal-business-summary', 'template.pptx'))
    const def = await analyzeTemplateBytes(bytes, { type: 'gorden-local' })
    ...
    const result = await measureTextFitReal(
      '长文本测量样本 sample text for measurement',
      slot,
      {

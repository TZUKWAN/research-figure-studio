...
export async function orchestrateFigure(
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    scientificIssues = auditScientific({
      plan: state.plan,
      placements: best.solve.placements,
      routes,
      importance: state.importance,
      ...(state.direction === 'TB' ? { direction: state.direction } : {}),
    })
    ...
    if (state.forbiddenHits.length > 0) {
      const message = `forbidden claim(s) reached the canvas: ${state.forbiddenHits.join('; ')}`

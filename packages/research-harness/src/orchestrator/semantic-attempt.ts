...
export interface CompileSemanticAttemptInput {
  ...
  /** P0.5 typography SSOT: resolved per-node pt the renderer will actually
   * draw. When present it overrides the raw spec sizes so measured geometry
   * matches the rendered figure. */
  typographyPt?: Record<string, { titlePt: number; detailPt: number }>
}
 ...
 * semantic change.
 */
function applyTypographyPt(
  spec: NodeTextSpec,
  nodeId: string,
  overrides?: Record<string, { titlePt: number; detailPt: number }>,
): NodeTextSpec {
  const pt = overrides?.[nodeId]
  if (!pt) return spec
  return { ...spec, titleSizePt: pt.titlePt, detailSizePt: pt.detailPt }
}
...
export function compileSemanticAttempt(input: CompileSemanticAttemptInput): SemanticAttemptState {
  ...
  const measured: MeasuredNode[] = plan.nodes.map((node) => ({
    ...measure(
      ...
      applyTypographyPt(input.specFor(node.type), node.id, input.typographyPt),
      estimatorMeasurer(),
    ),
  }))

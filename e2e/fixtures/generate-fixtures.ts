...
export async function ensureOwnedFixtures(): Promise<string> {
  ...
  // GordenDirProvider availability marker (INDEX.md) so the owned fixtures can
  // also serve as a Local Reference library in tests
  writeFileSync(
    join(FIXTURES_DIR, 'INDEX.md'),
  ...
  // GOAL §12: mirror the decks into the GordenDirProvider LAYOUT
  // (<library>/<deck>/template.pptx + INDEX.md), excluding the deliberately
  // broken deck — library specs point METIS_GORDEN_TEMPLATES_DIR here.
  const libraryDir = join(__dirname, 'local-reference')
  mkdirSync(libraryDir, { recursive: true })
  writeFileSync(
    join(libraryDir, 'INDEX.md'),
    '# Owned local-reference library (generated — project copyright)\n',
  )
  for (const name of Object.keys(FIXTURES)) {
    if (name === 'broken-template.pptx') continue
    const slug = name.replace(/\.pptx$/, '')
    const deckDir = join(libraryDir, slug)
    mkdirSync(deckDir, { recursive: true })
    copyFileSync(join(FIXTURES_DIR, name), join(deckDir, 'template.pptx'))
  }
  return libraryDir
}
